                                     ...
                                     FActorComponentTickFunction* ThisTickFunction)
{
	...
	{
		OnTargetingPlayerStatusChange.Broadcast(bHitPlayer);
	}
...
void UCombatComponent::FireTimerFinished()
{
	...
	if (CurrentWeapon->WeaponStatus == EWeaponStatus::Firing) CurrentWeapon->WeaponStatus = EWeaponStatus::Idle;
...
void UCombatComponent::Server_FireWeapon_Implementation(const FHitResult& Hit)
{
	...
	if(bHit)
	{
		bLethal = IPlayerInterface::Execute_DoDamage(Hit.GetActor(), bHeadShot ? CurrentWeapon->HeadShotDamage :  CurrentWeapon->Damage, GetOwner());
	}
...
void UCombatComponent::Equip(AWeapon* Weapon)
{
	...
	CurrentWeapon->AttachToOwingPawn(Cast<APawn>(GetOwner()));
...
void UCombatComponent::SetCurrentWeapon(AWeapon* NewWeapon, AWeapon* LastWeapon)
{
	...
	if (!IsValid(CurrentWeapon)) return;
	CurrentWeapon->AttachToOwingPawn(OwningPawn);
...
void UCombatComponent::InitalizeWeaponWidgets() const
{
	if (IsValid(CurrentWeapon))
	{
		OnReticleChanged.Broadcast(CurrentWeapon->GetReticleDynamicMaterialInstance(), CurrentWeapon->ReticleParams, bHitPlayer);
...
FRecoilStep UCombatComponent::GetCurrentRecoilStep() const
{
	...
	const FWeaponRecoil* Recoil = WeaponData->WeaponRecoil.Find(CurrentWeapon->WeaponType);
	
	FGameplayTag lastWeaponType = CurrentWeapon->WeaponType;
	

	if (!Recoil || Recoil->Pattern.IsEmpty())
	{
		return FRecoilStep();
	}

	const int32 Index = FMath::Min(CurrentRecoilIndex, Recoil->Pattern.Num() - 1);

	return Recoil->Pattern[Index];
}

void UCombatComponent::AdvanceRecoil()
{
	CurrentRecoilIndex++;
}
